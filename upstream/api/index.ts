...
function blockingImpactTasks(
  ...
  return [
    ...
    ...store.tasks
      ...
      .blocks.map((edge) => store.tasks.getTask(edge.dependentTaskId))
      .filter((dependent): dependent is StoredTask => dependent !== undefined),
  ];
}

function blockingSnapshot(
  store: TasksApiStore,
  tasks: readonly StoredTask[],
) {
